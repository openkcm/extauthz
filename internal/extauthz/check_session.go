...
func (srv *Server) checkSession(ctx context.Context, sessionCookie *http.Cookie, tenantID, method, host, path string) checkResult {
	...
	session, err := srv.sessionCache.LoadSession(ctx, tenantID, sessionCookie.Value)
	if err != nil {
	...
	intr, err := srv.jwtHandler.Introspect(ctx, session.Issuer, session.AccessToken, session.AccessToken, allowIntrospectCache)
	if err != nil {
