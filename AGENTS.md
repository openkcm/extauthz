...
- Tracing: `Check()` extracts W3C trace context (`traceparent` / `tracestate` / `baggage`) from `CheckRequest.Attributes.Request.Http.Headers` via the globally configured `TextMapPropagator`, then starts a server-kind span named `ext_authz.check` whose lifetime spans the entire call. The span carries `http.request.method`, `url.path`, `server.address`, plus `ext_authz.decision` (one of `ALLOWED` / `DENIED` / `UNAUTHENTICATED` / `TENANT_BLOCKED` / `UNKNOWN`) and `ext_authz.auth_type` (`x509` / `jwt` / `session` / `none`) set after the credential merge. Logs emitted during `Check()` automatically carry `TraceID` and `SpanID` via the `slogotel` prepender installed by `common-sdk/pkg/logger.InitAsDefault` — no per-call-site change needed. All of this is gated on `cfg.Telemetry.Traces.Enabled` (or any other `Telemetry.*.Enabled`) being true so that `common-sdk/pkg/otlp.Init` installs a real `TracerProvider` and `TextMapPropagator`; with telemetry off the span/extract operations degrade to no-ops.
